...
[<img src="https://raw.githubusercontent.com/machiav3lli/oandbackupx/main/badge_github.png" alt="Get it on GitHub" height="60">](https://github.com/TakeruF/designer-at-chrome/releases/latest)
...
- 通常遷移・SPA遷移でURLが変わったときの選択範囲とアウトラインの自動解除
- Basic、Typography、Colors、Spacing、Appearance、Layout、Structureの抽出
- 選択範囲内で文字を持つ要素だけを対象にした使用フォント一覧
...
- 設定画面から選べる、OS追従／ライト／ダークのカラーテーマ
- ブラウザ／OSの`AccentColor`へ追従するアクセント色（非対応時はChromeブルー）
- 日本語／英語／簡体字中国語の表示切り替え（初回はChromeのUI言語を使用）
- 設定画面から、サイトごとの許可（デフォルト）と全サイト一括許可を切り替え
- Inspectとデザインライブラリから、抽出した構造・スタイルを含むAI再現用プロンプトをコピー
...
- 選択要素・保存済みブックマークからの日英AI再現用プロンプト生成
...
| `<all_urls>`（任意権限） | `activeTab`失効後も、明示的な保存操作でスクリーンショットを撮影するため  |

`optional_host_permissions` には `<all_urls>` を宣言していますが、インストール時に全サイトへのアクセスは要求しません。この宣言から必要なoriginだけを部分的に要求できます。通常はツールバー操作で得る一時的な `activeTab` 権限を使います。別ドメインへ移動してSide Panelを使い続ける場合だけ、ユーザーが「このサイトを許可」を押した時点で、現在のoriginに限定した権限をChromeへ要求します。ヘッダーの設定ボタンから、希望するユーザーだけが全サイトアクセスを一括で許可できます。初期状態はサイトごとの許可です。

Chromeの仕様上、`captureVisibleTab()` は一時的な `activeTab` または文字どおりの `<all_urls>` 権限を必要とします。ページ移動後などに一時権限が失効した状態で保存した場合は、Side Panel内で理由を説明してから、スクリーンショット撮影用の任意権限を要求します。この許可は拒否でき、代わりに対象タブでUI Lensのツールバーアイコンを再クリックして一時権限を取得できます。許可後も撮影は「保存」操作時だけ実行します。

常時付与される `host_permissions` は設定していません。Content Scriptはmanifestで全ページに常駐させず、ユーザー操作後の現在タブへだけ注入します。任意権限と許可済みサイトはChromeの拡張機能設定からいつでも取り消せます。
...
- AI再現用プロンプトは抽出済みデータから端末内で生成し、ユーザー操作でクリップボードへコピーするだけです。AIサービスへの送信は行いません。
...
src/
  ...
  sidepanel/
    ...
    i18n.tsx                # 日本語／英語／簡体字中国語のローカル辞書とContext
  ...
  shared/
    ...
    reproduction-prompt.ts  # AI再現用プロンプトの日英ローカル生成
