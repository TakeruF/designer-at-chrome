...
export function explainProperty(name: string, value?: string): string {
  if (name === 'display' && value === 'flex') return '子要素をFlexboxで配置しています。';
  if (name === 'display' && value === 'grid') return '子要素を行と列のGridで配置しています。';
  if (name === 'position' && value === 'sticky') {
    return 'スクロール中、指定位置に達するとその位置に留まる要素です。';
  }
  if (name === 'position' && value === 'fixed') {
    return 'viewportを基準に固定され、スクロールしても同じ位置に表示されます。';
  }
  return glossary[name] ?? 'ブラウザが最終的に適用したCSSの値です。';
}
